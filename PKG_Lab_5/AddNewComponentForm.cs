...
public partial class AddNewComponentForm : Form
{
    ...
    public AddNewComponentForm(List<ResultNewComponentItem> newComponentItems, Bitmap bitmap, ComponentStorage container)
    {
        InitializeComponent();
        _newComponentItems = new List<ResultNewComponentItem>();
    ...
    private void add_Click(object sender, EventArgs e)
    {
        ...
        storage.AddComponent(new ComponentData(metaData, _newComponentItems[_index].Hash));

        // для отображения обнуружения образа
        FindedItems.Add(new ResultFindedItem(metaData,
            _newComponentItems[_index].Location, 100));
